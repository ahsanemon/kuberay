...
class VLLMDeployment:
    ...
    async def create_chat_completion(self, request: ChatCompletionRequest, raw_request: Request, token: str = Depends(verify_token)):
        if not self.openai_serving_chat:
            ...
            logger.info("OpenAI serving chat initialized")
        logger.info(f"Request: {request}")
        logger.info(f"Request tools: {getattr(request, 'tools', None)}")
        logger.info(f"Request tool_choice: {getattr(request, 'tool_choice', None)}")
        logger.info(f"Enable auto tools: {os.getenv('ENABLE_AUTO_TOOL_CHOICE', 'False')}")
        
        # Override tool_choice behavior if auto tools is disabled but tools are provided
        if (os.getenv("ENABLE_AUTO_TOOL_CHOICE", 'False').lower() not in ('true', '1', 't', 'yes') 
            and hasattr(request, 'tools') and request.tools 
            and not hasattr(request, 'tool_choice')):
            # Only use tools when explicitly needed, not automatically
            request.tool_choice = "none"
            logger.info("Auto tools disabled, setting tool_choice to 'none' for this request")
